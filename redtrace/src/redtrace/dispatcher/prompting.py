...
def add_blackboard_guidance(
    ...
    *,
    ...
    return (
        ...
        + "`.redtrace/artifacts/context`, while a deterministic high-signal summary and an evidence ID are returned. "
        + "For page captures, pass a stable `--source` URL so interactive-element, DOM, and network changes can be "
        + "reported without reinjecting prior HTML or screenshots. Use `redtrace-context query <evidence-id>` with "
        + "`--keyword`, `--lines`, or `--offset/--length`. Use `--length` when the next decision genuinely needs a "
        + "larger bounded slice; `REDTRACE_CONTEXT_QUERY_BYTES` exposes the configured ceiling. Do not artificially "
        + "restrict every query to the CLI's small convenience default, but still avoid rereading an entire Artifact "
        + "when a targeted slice is sufficient. Use `--passthrough` for progress-sensitive non-interactive commands; "
        + "bypass the harness for a truly interactive TTY. If the harness is disabled or fails before execution it "
        + "falls back to the raw command.\n\n"
        ...
        + "the relevant evidence ID/path. When compacting working context, remove repetition and stale process logs, "
        + "but retain the objective, scope, confirmed Facts, active direction, failed boundaries, authentication "
        + "state, evidence paths, unresolved constraints, and the next action."
    )
