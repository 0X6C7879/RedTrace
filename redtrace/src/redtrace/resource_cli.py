...
def _perform(args: argparse.Namespace) -> Any:
    ...
    if args.command == "capabilities":
        return {
            ...
            "workflow": [
                "call changes once with the retained audit_cursor when shared state may have moved",
                "filter with list --kind when you need a specific resource type",
                "get the selected resource by ID to inspect bounded metadata and declared actions",
                "run an action by resource ID; credential secrets are shared with Workers",
                "reuse an existing resource before registering a duplicate",
                "register shareable artifacts with the matching subcommand: webshell-create, session-register, credential-create with the secret on stdin, payload-import",
