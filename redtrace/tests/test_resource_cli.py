...
def _parse(*argv: str):
    return resource_cli.build_parser().parse_args(
        ["--server", "http://redtrace.test", "--project", "p001", "--worker", "codex-1", *argv]
    )
...
def test_listener_and_payload_commands_use_worker_aware_api(monkeypatch) -> None:
    ...
    oneliner = resource_cli._perform(
        _parse("payload-oneliner", "lis_123", "curl_beacon", "--callback-host", "edge.example.test")
    )
...
def test_snapshot_changes_and_waiting_run_are_bounded(monkeypatch) -> None:
    ...
    task_states = iter(
        [
            ...
            {"task": {"id": "op_123", "status": "succeeded", "output_summary": "uid=33"}},
        ]
    )
