...
def _worker(worker_type: str, env: dict[str, str]) -> WorkerConfig:
    return WorkerConfig.model_validate(
        {"name": worker_type, "type": worker_type, "task_types": ["bootstrap", "reason", "explore"], "max_running": 1, "priority": 0, "env": env}
    )
    ...
    config = DispatchConfig.model_validate(
        {
            ...
            "workers": [
                {"name": "ok-worker", "type": "mock", "task_types": ["bootstrap", "reason", "explore"], "max_running": 1, "priority": 0,
                 "env": {"MOCK_HEALTHCHECK": '{"delay":[0,0],"outcomes":{"ok":1.0,"fail":0.0}}'}},
                {"name": "fail-worker", "type": "mock", "task_types": ["bootstrap", "reason", "explore"], "max_running": 1, "priority": 0,
