...
class WorkerConfig(BaseModel):
    ...
    def validate_env(self) -> "WorkerConfig":
        # Pi always uses the configured benchmark gateway, including in local mode.
        if self.type == "pi":
...
class DispatchConfig(BaseModel):
    ...
    def validate_execution_mode(self) -> "DispatchConfig":
        if self.runtime.execution == "container" and self.container is None:
            raise ValueError("container config is required when runtime.execution is container")
        if self.runtime.execution == "local":
