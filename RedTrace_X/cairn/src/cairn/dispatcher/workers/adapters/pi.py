...
class PiDriver(WorkerDriver):
    ...
    def _wrap_with_models(self, worker: WorkerConfig, pi_argv: list[str], *, enable_tools: bool = True) -> list[str]:
        script = (
            ...
            "shift 2\n"
            'mkdir -p "$agent_dir"\n'
            'tmp="$agent_dir/models.json.$$"\n'
            'trap \'rm -f "$tmp"\' EXIT\n'
            'printf "%s" "$models_json" > "$tmp"\n'
            'mv "$tmp" "$agent_dir/models.json"\n'
    ...
    def _agent_dir(worker: WorkerConfig) -> str:
        return worker.env.get("PI_CODING_AGENT_DIR", "/home/kali/.pi/agent")

    @staticmethod
    def _session_dir(worker: WorkerConfig) -> str:
        return str(PurePosixPath(PiDriver._agent_dir(worker)) / "sessions" / worker.name)
