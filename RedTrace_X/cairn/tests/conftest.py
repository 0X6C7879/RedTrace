...
def pytest_collection_modifyitems(config, items) -> None:
    # Tests marked `posix` need a real POSIX shell / process groups (/bin/sh, os.killpg,
    # shebang CLIs). Skip them on Windows hosts; the WSL/POSIX run covers them for real.
    if sys.platform != "win32":
        return
    skip_posix = pytest.mark.skip(reason="POSIX-only: requires /bin/sh and process-group semantics")
    for item in items:
