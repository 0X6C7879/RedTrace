...
def test_local_common_env_reaches_worker_subprocess(tmp_path: Path, monkeypatch) -> None:
    ...
    worker = next(w for w in config.workers if w.type == "pi")
        ...
        "workers": [
            {
                ...
                "env": {
                    ...
                    "PI_CODING_AGENT_DIR": "/tmp/cairn-pi-test",
                },
            }
        ],
    }
...
def test_local_execution_uses_configured_pi_gateway() -> None:
    ...
    assert config.workers[0].env["PI_MODEL"] == "model"
...
def test_local_execution_requires_pi_gateway_env() -> None:
    payload = _local_payload()
...
def test_explore_runs_real_local_cli_end_to_end(tmp_path: Path, monkeypatch) -> None:
    ...
    config = _local_config_for_worker("test-worker", "pi")
