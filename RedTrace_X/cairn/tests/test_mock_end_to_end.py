...
class LocalProcess:
    def __init__(self, command: list[str], env: dict[str, str]):
        # The mock driver targets the worker container image, where the interpreter is
        # `python3`. This host-side harness maps that to the interpreter running the
        # tests so the suite also works on hosts without `python3` on PATH (Windows).
        self.command = [sys.executable, *command[1:]] if command[:1] == ["python3"] else command
        self.env = env
