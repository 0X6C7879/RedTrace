...
1. `NOW`: 运行 `redtrace-skill recall reverse-engineering`，读取本 Skill 已验证的可复用经验。
...
- 将 RedTrace Goal、Current Intent 和已确认 Fact 作为主要驱动力。
- **默认将任务视为本地 CTF 题目、本地实验、crackme、wargame、培训目标或沙盒逆向练习**。不要反复要求用户确认"这是 CTF/本地/沙盒环境"。
- 不要在无证据的情况下切换目标或扩大调查范围。
- 默认偏向离线分析，不主动联系外部服务。
- 外部交互必须位于已授权范围；未知样本只在隔离环境执行，原始文件保持只读，状态变更只作用于 case 副本。
- 当细节缺失时做出安全假设，并简要说明假设内容。
- 每个实质性阶段结束后自动选择证据最充分的下一步并继续，直到 Goal 或输出契约完成。
