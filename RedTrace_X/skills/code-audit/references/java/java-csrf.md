...
- 模式1：敏感删改接口（@PutMapping/@DeleteMapping）无 CSRF 校验
- 模式2：Spring Security CSRF 被禁用
- 模式3：Stateless 应用误用 Session
...
| Sink 点 | 说明 |
|---------|------|
| @PutMapping / @DeleteMapping | 敏感删改接口（重点检测） |
| @PostMapping | 需评估敏感度后再判定 |
| csrf().disable() | CSRF 禁用配置 |
...
# 检测删改接口（重点）
grep -rn "@PutMapping\|@DeleteMapping" --include="*.java"
