...
- 当 `file_path` 在 `{existing_file_paths}` 中时：
  - 若需确认该文件具体已有哪些 api_method，可用 `--file-exact` 精确查询：
    ```bash
    python3 $REDTRACE_SKILLS_DIR/code-audit/scripts/api_inventory_cli.py query --git <git_address> --file-exact <file_path> --output-file .code-audit-tmp/api_existing_{hash}.json
    ```
...
4. 执行插入命令：
   ```bash
   python3 $REDTRACE_SKILLS_DIR/code-audit/scripts/api_inventory_cli.py insert --file .code-audit-tmp/api_new_{随机后缀}.json
   ```
