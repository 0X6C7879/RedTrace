...
if [[ "${REDTRACE_SKIP_OPTIONAL_TOOLS:-0}" != "1" ]]; then
  ...
  install_skill_python_dependencies
  install_optional_language_tools
  if [[ "$OS" == "Linux" && "${REDTRACE_INSTALL_BENCHMARK_EXTRAS:-1}" == "1" ]]; then
    log "installing benchmark analysis, AI, chain, and offline-cache tools"
    CTF_VENV="$TOOL_VENV" \
      REDTRACE_DATA_DIR="$PROJECT_DIR/.redtrace/runtime/data" \
      REDTRACE_OFFLINE_DIR="$PROJECT_DIR/.redtrace/runtime/offline" \
      bash "$CTF_TOOL_INSTALLER" benchmark-extras
  fi
  verify_security_toolchain
else
...
Optional controls:
  ...
  REDTRACE_INSTALL_BENCHMARK_EXTRAS=0  Skip benchmark-only multi-chain/offline tools
