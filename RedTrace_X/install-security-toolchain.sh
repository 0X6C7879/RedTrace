...
#   python, gems, go, rsactftool, qiling, blockchain, ai, data,
#   benchmark-extras, all, --verify
  ...
  case "$manager" in
    apt)
      SYSTEM_PACKAGES=(
        ...
        python3 python3-venv python3-pip python3-dev openjdk-21-jdk-headless maven gradle
        libffi-dev libssl-dev libgmp-dev libmpfr-dev libmpc-dev zlib1g-dev liblzma-dev
    ...
    dnf|yum)
      SYSTEM_PACKAGES=(
        ...
        libffi-devel openssl-devel gmp-devel mpfr-devel libmpc-devel zlib-devel xz-devel
    ...
    zypper)
      SYSTEM_PACKAGES=(
        ...
        python3 python3-pip python3-devel java-21-openjdk-headless
        libffi-devel libopenssl-devel gmp-devel mpfr-devel mpc-devel zlib-devel xz-devel
    ...
    apk)
      SYSTEM_PACKAGES=(
        ...
        libffi-dev openssl-dev gmp-dev mpfr-dev mpc1-dev zlib-dev xz-dev
...
install_blockchain() {
  local asset_arch rust_arch tmp
  ...
  if [[ -n "$asset_arch" ]]; then
    ...
    if [[ "$FORCE" == false ]] && has forge && has cast && has anvil && has chisel; then
      ...
      tmp="$(mktemp -d)"
      if curl -fsSL "https://github.com/foundry-rs/foundry/releases/download/v1.2.0/foundry_v1.2.0_linux_${asset_arch}.tar.gz" -o "$tmp/foundry.tgz" \
    ...
    if [[ "$FORCE" == false ]] && has echidna; then
      ...
      tmp="$(mktemp -d)"
      if curl -fsSL "https://github.com/crytic/echidna/releases/download/v2.3.3/echidna-2.3.3-${rust_arch}-linux.tar.gz" -o "$tmp/echidna.tgz" \
        && tar -xzf "$tmp/echidna.tgz" -C "$tmp" echidna \
    ...
    # Aderyn
    if [[ "$FORCE" == false ]] && has aderyn; then
      SKIPPED+=("blockchain:aderyn")
    else
      tmp="$(mktemp -d)"
      if curl -fsSL "https://github.com/Cyfrin/aderyn/releases/download/aderyn-v0.6.8/aderyn-${rust_arch}-unknown-linux-gnu.tar.xz" -o "$tmp/aderyn.tar.xz" \
        && tar -xJf "$tmp/aderyn.tar.xz" -C "$tmp" --strip-components=1 "aderyn-${rust_arch}-unknown-linux-gnu/aderyn" \
        && cp "$tmp/aderyn" "$BIN_DIR/aderyn" && chmod +x "$BIN_DIR/aderyn"; then
        SUCCEEDED+=("blockchain:aderyn")
      else
        FAILED+=("blockchain:aderyn")
      fi
      rm -rf "$tmp"
    fi
    # heimdall-rs
    if [[ "$FORCE" == false ]] && has heimdall; then
      SKIPPED+=("blockchain:heimdall")
    else
      if curl -fsSL "https://github.com/Jon-Becker/heimdall-rs/releases/download/0.9.3/heimdall-linux-${asset_arch}" -o "$BIN_DIR/heimdall" \
        && chmod +x "$BIN_DIR/heimdall"; then
        SUCCEEDED+=("blockchain:heimdall")
      else
        FAILED+=("blockchain:heimdall")
      fi
    fi
  else
  ...
  if [[ "$FORCE" == false ]] && [[ -x "$mythril_venv/bin/myth" ]]; then
    ...
    if "$mythril_venv/bin/python" -m pip install "mythril==0.24.8" >>"$LOG_FILE" 2>&1; then
...
install_ai() {
  ...
  if has npm; then
    ...
    elif PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm install -g "promptfoo@0.104.0" >>"$LOG_FILE" 2>&1; then
...
install_benchmark_extras() {
  local installer="$PROJECT_DIR/container/install-benchmark-extras.sh"
  local targetarch
  [[ "$(uname -s)" == "Linux" ]] || {
    log_warn "benchmark extras currently support Linux only"
    SKIPPED+=("benchmark-extras:unsupported-os")
    return
  }
  [[ -f "$installer" ]] || {
    log_error "benchmark extras installer is missing: $installer"
    FAILED+=("benchmark-extras:installer")
    return
  }
  case "$(uname -m)" in
    x86_64|amd64) targetarch=amd64 ;;
    aarch64|arm64) targetarch=arm64 ;;
    *) log_error "unsupported benchmark architecture: $(uname -m)"; FAILED+=("benchmark-extras:arch"); return ;;
  esac
  if [[ "$DRY_RUN" == true ]]; then
    log_info "Would install the extended benchmark toolchain and curated offline caches"
    return
  fi
  TARGETARCH="$targetarch" \
  REDTRACE_BIN_DIR="$BIN_DIR" \
  REDTRACE_DATA_DIR="${REDTRACE_DATA_DIR:-$RUNTIME_DIR/data}" \
  REDTRACE_OFFLINE_DIR="${REDTRACE_OFFLINE_DIR:-$RUNTIME_DIR/offline}" \
  REDTRACE_SECURITY_VENV="$CTF_VENV" \
    bash "$installer" all >>"$LOG_FILE" 2>&1 \
    && SUCCEEDED+=("benchmark-extras:all") \
    || FAILED+=("benchmark-extras:all")
}
...
The benchmark-extras mode installs pwndbg, pycdc, ilspycmd, promptmap2,
Inspect/AgentThreatBench, AgentDojo, and the supported local chain toolchains.
...
run_mode() {
  ...
  case "$MODE" in
    ...
    ai) install_ai ;;
    benchmark-extras|extras) install_benchmark_extras ;;
    ...
    *)
      ...
      printf 'Usage: %s [--dry-run] [--force] {system|apt|dnf|yum|pacman|zypper|apk|brew|python|gems|go|rsactftool|qiling|blockchain|ai|data|benchmark-extras|all|--verify}\n' "$0" >&2
      return 2
      ;;
  esac
}
