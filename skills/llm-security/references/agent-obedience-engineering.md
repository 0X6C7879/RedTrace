...
| "记得沉淀可复用经验" | **MUST**: 经验写入产生该经验的专业 Skill（skill-evolution 负责执行，不由普通 Skill 重复触发） |
| "应该..." | **MUST** / **MUST NOT** |
...
| "这个工具我以前用过，知道路径" | **禁止猜测路径。** 必须用 `command -v` 获取实际路径，不同机器的安装位置不同。 |
| "任务已经基本完成了，不需要 checklist" | **任务完成的唯一定义是 Checklist 全部打勾。** 未完成 Checklist 的任务不算完成。 |
| "我没找到工具，我就直接猜路径" | **缺文件比猜错路径安全 100 倍。** 用 `command -v` 检测，缺工具由 RedTrace 统一部署逻辑补齐。 |
...
2. 我有没有猜过任何工具路径？如果有，`command -v` 检测到的实际路径是什么？
...
3. **NEXT**: 由宿主原生加载最匹配的一级 `skills/<name>/SKILL.md`
4. **NEXT**: 用 `command -v` 检测工具状态
