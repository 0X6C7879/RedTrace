...
1. `NOW`: 确认需要 **Ghidra**（无 IDA / 偏好开源 / 批量 headless）
2. `NEXT`: 用 `command -v` 检测 ghidra / ghidra-mcp 路径
3. `NEXT`: 缺工具 → bootstrap `ghidra-mcp`（若 manifest 支持）或按手动步骤装 Ghidra
4. `ACT`: 导入样本 → 自动分析 → 导出关键函数反编译
