...
| **代码审计 / SAST** | `code-audit/` | 白盒七模式：arch-scan / api-audit / mr-review / sast-audit / api-inventory / report-review / security-assessment，Java/Go/Python/JS，Codegraph 优先 |
| **静态 Finding 动态验证** | `code-audit-runtime-verify/` | Burp 流量 + 代码审计 + 黑盒分层验证（仅授权且已有静态 Finding） |
| **白盒基准归因** | `code-audit-benchmark/` | FP/FN 归因、流程缺陷、回归评估（仅评测时使用） |
