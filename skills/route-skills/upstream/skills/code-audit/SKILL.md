...
metadata:
  ...
  targetSkill: route-skills/code-audit
---
...
- **Skill 根目录**：脚本一律从当前文件位置推导 Skill 根目录（`SKILL_ROOT = Path(__file__).resolve().parents[1]`），不写死用户主目录；文档中的 `$REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/` 即本目录。
...
- **经验沉淀**：任务结束前，只有满足沉淀条件的新经验才通过 `$REDTRACE_SKILLS_DIR/route-skills/redtrace-tools/code-audit/evolve.py` 写入 `learned/`；项目事实只留在 Workspace，不写入本 Skill 全局目录。
...
| 输出格式校验（仅 api-audit/mr-review/report-review） | 仅这三模式适用：输出前运行 `node $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/validate-output.cjs <output.json> [mode]`（mode 取模式名），校验通过方可宣布完成；校验 schema 见 `$REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/output-schema.json`。**sast-audit/arch-scan/api-inventory/security-assessment 跳过本项**（输出形态非该 schema 覆盖） |
