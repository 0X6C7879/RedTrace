...
- **常规情况**：查询该文件全部 unprocessed API（无论数量多少，整个文件一次性处理）
  ```bash
  python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py query \
    --git <git_address> \
...
3. 执行批量更新命令：
   ```bash
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py update --git <git_address> --file .code-audit-tmp/api_updates_{随机后缀}.json
   ```
