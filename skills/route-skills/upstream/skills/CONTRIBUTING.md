...
- **用户可见消息/章节标签/报告/自动推进摘要**：使用中文（除非用户要求其他语言）。
...
### 3.4 自动推进模式（RedTrace Automation Pattern）

每个新建 skill 的工作流必须在阶段末尾定义可验证的完成条件、推荐的默认下一步和有界 fallback。Worker 根据 Goal、证据与失败记录自动选择并继续，不得依赖用户从菜单中选择。只有授权范围变化或缺少无法安全推断的必要输入时才允许暂停。
