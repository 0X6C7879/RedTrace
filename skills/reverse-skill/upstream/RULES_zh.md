...
当存在 `REDTRACE_SKILLS_DIR` 时，RedTrace 已将 `../REDTRACE_RULES.md` 作为 Claude、Codex、Pi 的全局覆盖规则注入。此运行模式不得修改宿主用户配置；任何菜单/确认门都由基于证据的自动推进替代。
...
- 同一工具自动安装失败 2 次后，停止重试，记录完整手动安装步骤并自动切换已安装的等价工具
- MCP 服务端口不一致时，自动探测配置与监听端口并更新运行时配置
...
- 发现高危漏洞时立即记录；在既有授权范围内继续验证，只有范围将改变时才暂停
- 不在报告或日志中保留未脱敏的敏感信息
...
| bootstrap 失败，原因明确 | 记录结构化引导，执行一次有界 fallback，然后继续 |
| bootstrap 失败，原因不明 | 检查一次网络/权限，随后自动切换已安装的等价工具 |
| 服务端口不一致 | 自动探测配置与监听端口并更新运行时配置 |
| 同一工具失败 2 次 | 明确告知"自动安装无法完成"，给完整手动步骤，不再重试 |
| 外部环境后来补齐工具 | 重新运行 refresh-tool-index.ps1（Linux / macOS 用 `bash skills/scripts/refresh-tool-index.sh`）验证，然后继续 |
...
| Agent 常见借口 | 反驳（强制执行） |
|---|---|
| "这一步可以省略，我直接..." | **禁止静默跳过。** 记录具体原因，自动选择最强的有效 fallback 并继续。 |
| "根据我的判断，这不是必需的" | **你的判断在此处不适用。** 列出你用来判断的具体标准，并解释为什么这个标准允许跳过明确写出的步骤。 |
| "用户大概不需要这个" | **按 Goal 和证据执行。** 自动选择范围内最强动作，不得把备选项变成阻塞菜单。 |
...
| 借口 | 反驳 |
|------|------|
| "这一步可以省略" | 禁止静默跳过；记录原因并执行最强有效 fallback |
| "用户大概不需要这个" | 按 Goal 和证据自动选择范围内最强动作 |
