...
<img src="./README/banner.png" alt="RedTrace Banner"/>
...
RedTrace is a general-purpose problem-solving engine. <br/>It defines no roles, no workflows. Given an origin and a goal, it searches for a path through an unknown state space. <br/>AI Penetration Testing is one such problem — and a proven one.
...
<p align="center">
  <a href="https://www.bilibili.com/video/BV1a8R5BhEVi/" target="_blank" rel="noopener noreferrer">
    <img src="./README/redtrace.png" alt="RedTrace runtime screenshot" width="900" />
  </a>
</p>
...
RedTrace is built for this class of problems. Penetration testing is the first domain it has been validated on.
...
**RedTrace Server** maintains graph consistency only.

**RedTrace Dispatcher** reads the graph, schedules tasks, spins up and tears down worker containers, and is the sole writer to the protocol. Each project gets its own Worker Container; multiple Agent Workers run concurrently inside it. Agent Workers only receive a prompt and return structured output.
...
RedTrace keeps reusable agent capabilities in two project-root directories:

- `./skills/<name>/SKILL.md` is the single Agent Skills source for Claude Code,
  Codex, and Pi.
- `./mcp/<name>.json` is the single MCP server source. Common fields are
  translated to each agent's native configuration, while optional
  `agents.claude`, `agents.codex`, and `agents.pi` objects preserve
  runtime-specific fields.

The dispatcher synchronizes enabled resources into every project workspace
before an agent starts. Claude uses `.claude/skills` and `--mcp-config`, Codex
uses `.agents/skills` and native `mcp_servers` config, and Pi uses
`.agents/skills` plus its project MCP extension. Existing user/project
configuration remains enabled; RedTrace does not apply a Skill, MCP, or extension
allowlist.

Open the **Skills** and **MCP** pages in the Web UI to create, edit, enable,
disable, delete, and inspect the shared resources. Docker Compose mounts both
directories read/write so changes persist on the host and are visible to the
server and dispatcher.

### On-demand shared blackboard

Running Claude Code, Codex, and Pi workers also receive the same read-only `redtrace-blackboard` CLI. An agent may use it when fresher shared context would help, but RedTrace never pushes blackboard changes, polls on the agent's behalf, interrupts a task, or injects new nodes into model context automatically.
...
redtrace-blackboard context i002 --depth 1
```

Commands return bounded JSON and inherit project, worker, task, Intent, server, and snapshot revision from the Worker runtime. Queries are audited without changing the blackboard revision. See [On-demand Worker blackboard access](docs/shared-blackboard-cli.md) for command and protocol details.
...
- <a href="https://mp.weixin.qq.com/s/2rEqFLvkxvYWM3gW170C2w" target="_blank" rel="noopener noreferrer">The Pathless Path: RedTrace AI from Penetration Testing to General Problem Solving</a>
...
```bash
docker pull --platform=linux/amd64 ghcr.io/oritera/redtrace-worker-container:latest
```
...
Pull the base image used to build RedTrace:
...
This starts `redtrace-server` on port `8000` and `redtrace-dispatcher` once the server passes its health check. The dispatcher mounts `dispatch.yaml` from the project root and connects to Docker via the host socket. Data is persisted to `./datas/redtrace/`.
...
uv run --project redtrace redtrace dispatch --config dispatch.yaml --startup-healthcheck-only
```
...
RedTrace is a general-purpose problem-solving engine. Although it supports penetration testing, CTF solving, security assessment, and vulnerability research workflows, it is intended to be used only in environments where you have explicit authorization to operate.

You are solely responsible for how you use this project. Do not use RedTrace against systems, networks, applications, or data without clear prior permission from the owner or operator. Unauthorized security testing, exploitation, or data access may be illegal and may cause harm.
...
<a href="https://www.star-history.com/#oritera/RedTrace&Date" target="_blank" rel="noopener noreferrer">
  <img src="https://api.star-history.com/svg?repos=oritera/RedTrace&type=Date" alt="Star History Chart" />
</a>
