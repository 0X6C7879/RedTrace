## RedTrace Burp Suite 插件（中文说明）

> 本源码由 CyberStrikeAI Burp Suite 插件迁移而来，由
> `RedTrace/plugins/manifest.json` 统一管理。默认连接
> `http://127.0.0.1:8000`；仅在服务端设置 `REDTRACE_PLUGIN_TOKEN` 时填写令牌。
