...
SEARCH_PATHS=(
  ...
  # Homebrew formula locations
  "/opt/homebrew/opt/ghidra"
  "/usr/local/opt/ghidra"
  "/home/linuxbrew/.linuxbrew/opt/ghidra"
  # Manual installation locations
  "/opt/ghidra"
  "/usr/local/ghidra"
  "$HOME/.local/share/redtrace-tools/ghidra"
  "$HOME/ghidra"
...
for base_path in "${SEARCH_PATHS[@]}"; do
  if [[ -d "$base_path" ]]; then
    ...
    HEADLESS=$(find -L "$base_path" -name "analyzeHeadless" -type f 2>/dev/null | head -n 1)
