...
| **R26** | 代码审计 / SAST / Semgrep / PR / MR / API 源码审计 | `code-audit/` |
...
| **R40** | 静态 Finding 需要测试环境动态验证 / Burp 验证告警 | `code-audit-runtime-verify/` |
| **R41** | 白盒准确率 / FP / FN / 流程诊断 / 回归评估 | `code-audit-benchmark/` |
| **R0** | 通用逆向 / 反调试 / OLLVM / 未知二进制 | `reverse-engineering/` |

未命中强关键词 → PRIMARY=`R0`，并提示打开 `routing.md`。

## code-audit 模式细分（R26 内部路由）

```text
代码仓库首次理解          -> code-audit + arch-scan
指定 API 审计            -> code-audit + api-audit
PR/MR/Diff 审查          -> code-audit + mr-review
CodeQL/Semgrep/SAST 告警 -> code-audit + sast-audit
API 盘点                -> code-audit + api-inventory
已有报告复核             -> code-audit + report-review
整体安全评估             -> code-audit + security-assessment

静态 Finding 需要测试环境验证   -> code-audit-runtime-verify
白盒准确率、FP/FN 和流程诊断   -> code-audit-benchmark
```

RedTrace 阶段映射：`bootstrap` → arch-scan / api-inventory；`explore` → api-audit / mr-review / sast-audit；独立验证 Intent → report-review / runtime-verify；`conclude` → security-assessment。
