...
```bash
python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_discovery_cli.py <repo_path> --git <git_address> [options]
```
...
```bash
python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py <command> [options]
```
...
> **本模式不适用通用 JSON 输出校验脚本**（`$REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/validate-output.cjs`）：api-inventory 输出为 API 库存数据而非漏洞 finding 结构，schema 不覆盖。跳过 SKILL.md 通用质量门禁的「输出格式校验」项。
