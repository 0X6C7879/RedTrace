...
1. 获取当前库存的文件分组统计（用于后续对比）
   ```bash
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py query --git <git_address> --summary
   ```
...
- 若有新增文件，用 `--file-exact` 抽查新文件详情，验证字段质量：
  ```bash
  python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py query --git <git_address> --file-exact <new_file_path> --output-file .code-audit-tmp/api_verify_new.json
  ```
...
2. **检测重复接口**：用 `--find-duplicates` 查找重复记录
   ```bash
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py query --git <git_address> --find-duplicates
   ```
...
3. **删除重复记录**：**执行删除前必须检查 `api_path` 是否相同**：
   ...
   ```bash
   # 预览（--git 必填）
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py delete --git <git_address> --ids <duplicate_ids逗号分隔> --dry-run
   # 确认删除
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py delete --git <git_address> --ids <duplicate_ids逗号分隔> --confirm
   ```
...
1. 用 `--summary --status unprocessed` 获取有未处理 API 的文件列表（只返回 unprocessed > 0 的行，已全部处理的文件不出现）
   ```bash
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py query --git <git_address> --status unprocessed --summary
   ```
...
```bash
python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py query --git <git_address> --file-exact <file_path> --output-file .code-audit-tmp/api_verify_<file_name>.json
```
验收要点（逐条检查，**不合格需重试**）：
...
1. 用 `stats --include-files` 命令获取完整统计数据（含文件分组）：
   ```bash
   python3 $REDTRACE_SKILLS_DIR/route-skills/upstream/skills/code-audit/scripts/api_inventory_cli.py stats --git <git_address> --include-files
   ```
