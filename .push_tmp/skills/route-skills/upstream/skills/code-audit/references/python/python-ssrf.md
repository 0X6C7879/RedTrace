...
**强制执行顺序**：
1. **首先**：找到 sink 点 URL 最终构造代码（如 `url = ...`）
...
| **风险-B** | 有 HTTP 入口可达，但防护不充分 | 1. 存在网络请求; 2. HTTP 入口可达; 3. 仅有弱防护（startswith/contains 可绕过） |
...
| Host 部分 | `url = f"http://{input}/api"` | 需继续研判 |
| 完整 URL | `url = input` 或 `requests.get(input)` | 需继续研判 |
...
- url = f"http://fixed.com/{user_input}"        ← ✅ 前缀以 / 结尾，@ 注入不可行
- url = base_url + "/" + user_input              ← ✅ 需确认 base_url 含 scheme://host/ 形式
...
Step 4: URL/Host 来源检查
  ├─ 来自 Kconf/Django settings/环境变量/数据库？ → 安全
  └─ 用户直接输入 → 继续

Step 5: 内网 IP 检测
  ├─ 有 is_internal_ip/ipaddress 私网检测？ → 安全
  └─ 无检测 → 继续

Step 6: 白名单检查
  ├─ 域名白名单（精确匹配）？ → 安全
  ├─ 白名单但 startswith/contains？ → 漏洞（可绕过）
  └─ 无白名单 → 继续
...
| URL 来自 Kconf/Django settings/环境变量/数据库 | 漏洞 | 安全 |
| ipaddress 私网检测 | 漏洞 | 安全 |
| 域名白名单（精确匹配） | 漏洞/风险 | 安全 |
...
| Host 可控 + 无防护 | 漏洞 |
| 白名单用 startswith/contains | 漏洞（可绕过） |
| 内部函数无 HTTP 入口 | 风险-A |
...
### 场景1：完整 URL 用户输入 / Host 拼接

```python
url = request.GET.get('url')
requests.get(url)  # 漏洞：完整 URL 用户可控

url = f"http://{request.GET.get('host')}/api"
requests.get(url)  # 漏洞：Host 用户可控
```
...
if url.startswith("http://internal.com"):
    requests.get(url)  # 漏洞：startswith 可被绕过

if "example.com" in url:
    requests.get(url)  # 漏洞：contains 可被绕过
```
...
url = f"http://fixed.host.com/{path}"
requests.get(url)  # 安全：Host 固定
```

### 隔离代理

```python
SSRF_PROXY = {'http': 'http://ssrf_proxy:8080', 'https': 'http://ssrf_proxy:8080'}
requests.get(url, proxies=SSRF_PROXY)  # 安全
```
...
requests.get(url)  # 安全：攻击者无法控制白名单域名的 DNS 解析
```

### Django settings / Kconf 配置

```python
BASE_URL = settings.API_BASE_URL  # Django settings
BASE_URL = config.get('api.base.url')  # Kconf
url = f"{BASE_URL}/{path}"
requests.get(url)  # 安全：Host 来自配置
```

### 数据库查询 URL

```python
config = CallbackConfig.objects.get(id=callback_id)
requests.get(config.url)  # 安全：管理员配置的可信 URL
```
...
| 类型 | 关键词 |
|------|--------|
| HTTP 客户端 | `requests.get`, `requests.post`, `urllib.request.urlopen`, `httpx.get`, `aiohttp` |
| URL 解析 | `urlparse`, `urllib.parse` |
| 隔离代理 | `ssrf_proxy`, `anti_ssrf`, `SSRF_PROXY` |
| 校验函数 | `ALLOWED_HOSTS`, `is_internal_ip`, `WHITE_LIST` |
| 配置来源 | `settings.`, `os.getenv`, `config.get`, `environ` |
...
# 检测隔离代理
grep -rn -i "ssrf\|anti.*proxy" --include="*.py"

# 检测白名单
grep -rn "ALLOWED_HOSTS\|WHITE_LIST" --include="*.py"
```
...
**错误**: 只看数据流流向 `requests.get`
**正确**: 必须回溯到 URL 构造行，分析用户输入在 URL 的哪个位置

### 陷阱3：代理名称不含 anti

**错误**: 看到代理就认为有 SSRF 防护
**正确**: 只有名称包含 `anti` 或 `ssrf` 的代理才是隔离代理
...
| 场景 | 判定 | 原因 |
|------|------|------|
| 内网 IP（10.x/172.16-31.x/192.168.x） | 不报告 | 正常的内网服务调用 |
| 内网域名（*.internal/*.local） | 不报告 | 服务间通信 |
| localhost/127.0.0.1 | 不报告 | 本地服务调用 |
| 硬编码的外网 API | 不报告 | 正常的第三方服务调用 |
| URL 来自 Kconf/settings/环境变量 | 不报告 | 配置中心，用户无法修改 |
...
```python
# urllib.request 支持特殊协议（requests 仅 HTTP/HTTPS，相对安全）
# file:///etc/passwd
...
域名白名单通过后，`requests` 默认 `allow_redirects=True` 会跟随 302 到内网。

```python
# 防护：禁用重定向
requests.get(url, allow_redirects=False)
# 或使用隔离代理（自动拦截重定向到内网的请求）
```
...
- [ ] 隔离代理已确认名称含 anti/ssrf，非仅凭"有代理"判断
- [ ] 白名单实现已检查（精确匹配 vs startswith/contains）
- [ ] HTTP 入口可达性已确认，非假设
- [ ] 已读取 URL 最终构造代码和校验函数实现（禁止推测）
- [ ] 已应用误报排除规则（内网 IP/域名/localhost/硬编码 API 不报告）
- [ ] 不因代码对比而判定风险
- [ ] 结论与证据一致，代码行号可追溯
