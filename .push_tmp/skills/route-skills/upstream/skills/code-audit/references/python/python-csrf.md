...
- 模式1：敏感删改接口（PUT/DELETE）无 csrf_token
- 模式2：@csrf_exempt 装饰器使用
- 模式3：CsrfViewMiddleware 被禁用
...
# 检测删改路由（重点）
grep -rn "@app.route.*methods.*PUT\|@app.route.*methods.*DELETE\|@app.route.*methods.*PATCH" --include="*.py"

# 检测 POST 视图（需评估敏感度）
grep -rn "request.POST\|@app.route.*methods.*POST" --include="*.py"
```
